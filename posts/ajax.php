...
switch ($type) {
    ...
    case "delete_video": // addCategory.php
        ...
        break;
    case "reembolsar_course": // addCategory.php
        try {
            $course_id = (isset($_POST['course_id'])) ? $_POST['course_id'] : '';
            $course_id = intval($course_id);
            ...
            if ($sql->rowCount() > 0) {
                foreach ($sql->fetchAll() as $value) {
                    ...
                    $access = $value['access'];
                    if ($access != "") {
                        $data_arr = json_decode($access, true);
                        $chave = array_search($course_id, $data_arr);
                        if ($chave !== false) {
                            unset($data_arr[$chave]);
                        }
                    }
                    
                    $encoded = json_encode(array_values($data_arr));
                    
                    
                    
                    $sql = "UPDATE users SET access = '$encoded' WHERE username = '$username'";
                    $sql = $pdo->prepare($sql);
                    $sql->execute();
                    $status = 1;
                }
